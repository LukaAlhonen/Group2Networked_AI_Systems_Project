"""
This is the enter point for the root cluster
"""
import sys
import os
from schedule import schedule, task_creation
import tkinter as tk
import pandas as pd
import socket
import pickle
from pprint import pprint

if __name__ == "__main__":
    # Path of test file should be transferred by arguments in sys.argv or from environment variable 'batch_path'
    args = sys.argv
    if 'batch_path' in os.environ:
        path = os.environ['batch_path']
    elif len(args) == 2:
        path = args[1]
    else:
        raise ValueError("Please provide 1 absolute path of the batch!")
    
    df = pd.read_parquet(path)
    pids = schedule(df)

    if pids == "alarm":
        sys.exit(-1)

    serialized_data = task_creation(pids, df)

    # Socket Programming part - Transfer serialized_data from Root to Worker
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    if 'worker_address' not in os.environ and 'worker_port' not in os.environ:
        print('No address or port specified, binding to "localhost:8001"')
        address = 'localhost' # Replace with worker IP
        port = 8001
    else:
        address = os.environ['worker_address']
        port = os.environ['worker_port']
    s.connect((address, port))
    print(f'Connected to {address}:{port}')
    print('Sending serilazied data...')
    s.send(serialized_data)
    s.shutdown(socket.SHUT_WR) # Close sending side of socket

    # Wait for response from worker
    response = b''
    print('Waiting for response...')
    try:
        while True:
            chunk = s.recv(1024)
            if not chunk:
                break
            response += chunk
    except:
        print('Something unexpected occured...')
    
    # Deserialise response and print rows containing anomalies
    deserialised_response = pickle.loads(response)
    print(deserialised_response)
    for idx in deserialised_response:
        pprint(df.iloc[idx].to_dict())
    print('Done.')
    s.close()
    
    